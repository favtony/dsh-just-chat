...
import { createElement, useCallback, useEffect, useMemo, useRef, useState, useSyncExternalStore, type RefObject } from 'react'
...
import { SIDEBAR_SECTION_STYLE, SidebarErrorBoundary, SidebarLayoutGuard } from './sidebar-layout-guard.tsx'
...
type OfficialWorkspaceBrowser = React.ComponentType<WorkspaceBrowserProps>

interface CombinedSidebarContentProps {
  deps: SidebarBrowserAdapterDeps
  official: OfficialWorkspaceBrowser
 ...
 */
function CombinedSidebarContent({ deps, official, props, rootRef }: CombinedSidebarContentProps): React.ReactElement {
  ...
  return createElement(
    ...
    createElement(
      'div',
      { 'data-dsh-just-chat-section': 'workspaces', style: SIDEBAR_SECTION_STYLE },
      createElement(official, workspaceProps),
    ),
    props.wide
      ? createElement(
          'div',
          { 'data-dsh-just-chat-section': 'conversations', style: SIDEBAR_SECTION_STYLE },
          createElement(official, conversationProps),
        )
      : null,
  )
}
...
export function createSidebarBrowserAdapter(
  ...
  return official => function SidebarBrowserAdapter(props): React.ReactElement {
    ...
    const officialComponent = official as OfficialWorkspaceBrowser
    const fallback = createElement(officialComponent, props)

    if (disabled) return fallback

    const pluginLayout = createElement(
      CombinedSidebarContent,
      { deps, official: officialComponent, props, rootRef },
    )
    return createElement(
